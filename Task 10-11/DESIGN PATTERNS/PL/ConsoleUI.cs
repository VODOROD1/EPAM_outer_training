    ...
    class ConsoleUI : IUI
    {
        ...
        public void AskDeleteUser()
        {
            while (true)
            {
                ...
                if (char.IsDigit(input) && (uint)Char.GetNumericValue(input) <= UsersAwardsManager.GetAllUsers().Count)
                {
                    DeleteUser(input.ToString());
                    break;
                }
                else { Console.WriteLine("Введите корректное число!"); }
            }
        ...
        {
            UsersAwardsManager.DeleteUser(input.ToString());
        }
